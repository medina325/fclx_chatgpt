...
func (c *ChatService) ChatStream(req *pb.ChatRequest, stream pb.ChatService_ChatStreamServer) error {
	...
	ctx := stream.Context()
	// Criando função para rodar em outra thread.
	// Essa função fica rodando enquanto houver novas mensagens
	// chegando via stream, que estao sendo geradas pelo método Execute
	go func() {
