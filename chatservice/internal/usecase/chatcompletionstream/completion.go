...
// É uma struct que apenas guarda estado, usada para transferir dados
// do mundo externo para o mundo interno, e vice-versa
// Usamos esse cara, para aplicar o conceito de injeção de dependência
// essa que virá, muito provavelmente, a partir de um controller
...
type ChatCompletionUseCase struct {
	...
	Stream       chan ChatCompletionOutputDTO // chan de channel
}
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	if err != nil {
		return nil, errors.New("error cereating chat completion: " + err.Error())
	}
	...
	for {
		...
		// Agora queremos mandar para uma thread de comunicação
		// Em GO, estamos falando de um "channel"
		// Lá no gRPC vamos pegar esses dados em stream
		uc.Stream <- r // Que sintaxe foda ein
	}

	// Hora de guardar a mensagem enviada, no banco de dados, para mantermos o histórico
